...
async function main() {
    ...
    const payRuns = await Promise.all([
        prisma.payRun.create({
            data: {
                ...
                status: "APPROVED",
                createdById: users.find((u) => u.companyId === company1.id)?.id || null,
                approvedById: users.find((u) => u.companyId === company1.id)?.id || null,
            },
        }),
        prisma.payRun.create({
            data: {
                ...
                status: "APPROVED",
                createdById: users.find((u) => u.companyId === company2.id)?.id || null,
                approvedById: users.find((u) => u.companyId === company2.id)?.id || null,
            },
        }),
        prisma.payRun.create({
            data: {
                ...
                periodEnd: new Date("2025-09-30"),
                status: "DRAFT",
                createdById: users.find((u) => u.companyId === company3.id)?.id || null,
            },
        }),
    ]);
    // --------------------------
    ...
    for (const payRun of approvedPayRuns) {
        ...
        for (const payslip of payslips) {
            const cashier = users.find((u) => u.companyId === payRun.companyId && u.role === 'CASHIER');
            if (!cashier)
                continue;
