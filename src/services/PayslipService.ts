...
export class PayslipService {
    ...
    async generatePDF(payslipId: number): Promise<string> {
        ...
        doc.fillColor('white').fontSize(12).font('Helvetica-Bold').text(
            `Période: ${payslip.payRun.periodStart.toLocaleDateString('fr-FR')} - ${payslip.payRun.periodEnd.toLocaleDateString('fr-FR')}`,
            60, doc.y - 20, { align: 'center' }
        );
        doc.moveDown(2);
        ...
        const drawRow = (label: string, amount: number, isEven: boolean) => {
        ...
        doc.fillColor('white').fontSize(11).font('Helvetica-Bold').text(
            (payslip.netPayable - payslip.paidAmount).toLocaleString('fr-FR'), 260, currentY + 7
        );
        ...
        doc.fillColor(statusColor).fontSize(11).font('Helvetica-Bold').text(
            payslip.status === 'PAID' ? 'PAYÉ' : payslip.status === 'PENDING' ? 'EN ATTENTE' : 'NON PAYÉ'
        );
        ...
        doc.fillColor(secondaryColor).fontSize(8).font('Helvetica').text(
            'Ce document est généré automatiquement par le système de gestion des salaires.',
            { align: 'center' }
        );
